...
arff.dump('./data_weka_EVOO_nonEVOO/val.arff', val_scaled.values, relation = 'val', names = val_scaled.columns)

for i in range(5):
    train, opt = train_test_split(train_aux_scaled, test_size = val_scaled.shape[0], random_state = i, stratify = train_aux_scaled['Class'])
    arff.dump('./data_weka_EVOO_nonEVOO/'+str(i)+'.arff', train.values, relation = 'train', names = train.columns)
    arff.dump('./data_weka_EVOO_nonEVOO/'+str(i+5)+'.arff', opt.values, relation = 'opt', names = opt.columns)
